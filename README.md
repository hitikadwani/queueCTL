...
- The database file (`queue.db`) is created automatically in the current directory
- Workers should be stopped using Ctrl+C in the terminal where they're running
- Commands that don't exist or fail will trigger retries
...
- Check if jobs have `next_retry_at` in the future (for failed jobs)

**Database locked errors:**
- Ensure only one process is accessing the database at a time
- Close any database connections properly

**Jobs stuck in processing:**
- Manually unlock by restarting workers (they will unlock on shutdown)
